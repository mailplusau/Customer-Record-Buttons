...
function beforeUserLoad(type, form) {
	if (type == "view") {
		...
		var signStatus = ["22", "31", "48", "13", "80"]; // to be addressed to a process management record
		//22 - Customer - Lost,
		//31 - Free Trial - Lost,
		...
		var customerRecord = nlapiLoadRecord(
			nlapiGetRecordType(),
			customerRecordId
		);
		var status = nlapiGetFieldValue("entitystatus");
		var syncedWithProspectPlus = nlapiGetFieldValue(
			"custentity_lead_synced_to_firebase"
		);
		...
		var cancellation_reason = nlapiGetFieldValue(
			"custentity_service_cancellation_reason"
		);
		...
		var parentCompanyID = nlapiGetFieldValue("parent");
		var syncedWithLPOPlus = nlapiGetFieldValue("custentity_lpo_synced_with_db");
		var partnerId = nlapiGetFieldValue("partner");
		var lpoParentAccount = nlapiGetFieldValue("custentity_lpo_parent_account");
		...
		if (isInArray(status, signStatus)) {
			if (isInArray(userRole, systemAdmin)) {
				form.addButton(
					"custpage_send_email",
					"Send Email",
					getButtonScript("customer_send_email", "null", customerRecordId)
				);
				if (
					!isNullorEmpty(
						customerRecord.getFieldValue("custentity_display_name")
					)
				) {
					form.addButton(
						"custpage_rta_sync_display_name",
						"Sync RTA Display Name",
						getButtonScript("rta_sync_display_name", "null", customerRecordId)
					);
				}
				if (
					companyName.slice(-12) === "LPO - Parent" &&
					isNullorEmpty(parentCompanyID) &&
					syncedWithLPOPlus == 1
				) {
					nlapiLogExecution(
						"AUDIT",
						"Adding Resync LPO Button",
						"Conditions met for Resync LPO button to be added"
					);
					form.addButton(
						"custpage_resync_lpo",
						"Resync LPO with LPO.plus",
						getButtonScript("resync_lpo", "null", customerRecordId)
					);
				}
				...
				if (
					...
					nlapiLogExecution(
						"AUDIT",
						"Adding Resync LPO Button (non-parent company)",
						"Conditions met for non-parent company Resync LPO button"
					);
					form.addButton(
						"custpage_resync_lpo_sub_customer",
						"Resync with LPO.plus",
						getButtonScript("resync_lpo_sub_customer", "null", customerRecordId)
					);
				}
			}
			form.addButton(
				"custpage_update_customer",
				"Update Customer",
				getButtonScript("update_customer", "null", customerRecordId)
			);

			if (syncedWithProspectPlus == 1) {
				form.addButton(
					"custpage_sync_from_prospectplus",
					"Sync from ProspectPlus",
					getButtonScript("sync_from_prospectplus", "null", customerRecordId)
				);
			}
			...
			if (isInArray(userRole, salesRoles)) {
				var link =
					"/app/common/entity/custjob.nl?pf=CUSTENTITY_REF_REFERRER&pr=-2&stage=lead&pi=" +
				...
				var fil_sr = [];
				fil_sr[fil_sr.length] = new nlobjSearchFilter(
					"custrecord_sales_completed",
					null,
					"is",
					"F"
				); // not completed
				fil_sr[fil_sr.length] = new nlobjSearchFilter(
					...
					"is",
					[1, 2]
				); //campaignrecordtype is 'Existing Customer'
				fil_sr[fil_sr.length] = new nlobjSearchFilter(
					"custrecord_sales_inuse",
					null,
					"isnot",
					"T"
				); // not in use
				...
				fil_sr[fil_sr.length] = new nlobjSearchFilter(
					"custrecord_sales_deactivated",
					null,
					"is",
					"F"
				); // is not deactivated sales records

				var fil_sr2 = [];
				fil_sr2[fil_sr2.length] = new nlobjSearchFilter(
					"custrecord_sales_campaign",
					null,
					"is",
					50
				); // AusPost - Product - Price Increase
				fil_sr2[fil_sr2.length] = new nlobjSearchFilter(
					"custrecord_sales_customer",
					null,
					"is",
				...
				col_sr[col_sr.length] = new nlobjSearchColumn(
					"custrecord_sales_assigned"
				); //lastassigned
				col_sr[col_sr.length] = new nlobjSearchColumn(
					"custrecord_sales_outcome"
				); // lastoutcome
				col_sr[col_sr.length] = new nlobjSearchColumn(
					"custrecord_sales_campaign"
				); // campaign
				...
				var linkedMultisiteSearch = nlapiLoadSearch(
					"customer",
					"customsearch_linked_multisite_records"
				);

				var addFilterExpression = new nlobjSearchFilter(
					"internalId",
					null,
					"anyof",
					customerRecordId
				...
				if (res_sr2 == null) {
					var lpo_customer = customerRecord.getFieldValue(
					...
					if (res_sr == null) {
						...
						// if (status == 13) {
						form.addButton(
							"custpage_customer_create_sales_record",
							"Create Sales Record",
							getButtonScript("cr8custsalesrec", null, customerRecordId)
						);
						// }
						if (userRole == 3) {
							form.addButton(
								"custpage_customer_create_sales_record_new",
								"Internal Qualification",
								getButtonScript(
									"create_sales_record_new",
									null,
									customerRecordId
								)
							);
						...
						if (
							lastassigned == nlapiGetUser() ||
							isEmpty(lastassigned) ||
							isInArray(lastassigned, nonActiveSalesRep) ||
							(debugging && userRole == 3)
						) {
							...
							var campaignRecord = nlapiLoadRecord(
								"customrecord_salescampaign",
								campaignID
							);

							var campaignType = campaignRecord.getFieldValue(
								"custrecord_salescampaign_recordtype"
							);
			...
			if (isInArray(userRole, systemAdmin)) {
				...
				if (status == 13 || status == 32) {
					form.addButton(
						"custpage_prod_pricing",
						"Create Product Pricing",
						getButtonScript("customer_prod_pricing", null, customerRecordId)
					);

					if (userRole == 3) {
						form.addButton(
							"custpage_sync_prod_pricing",
							"Sync Product Pricing",
							getButtonScript(
								"customer_sync_prod_pricing",
								null,
								customerRecordId
							)
						);
			...
			if (status == 13 || status == 32) {
				form.addButton(
					...
					getButtonScript("customer_change", null, customerRecordId)
				);
				if (isInArray(userRole, systemAdmin)) {
					form.addButton(
						"custpage_cancellation",
						"Cancel Customer",
						getButtonScript("customer_cancellation", null, customerRecordId)
					);
				}
			...
			if (maapnumber != null) {
				...
				if (isInArray(userRole, financeRoles)) {
					form.addButton(
						"custpage_generatemaap",
						"Generate MAAP #",
						getButtonScript("maapgen", null, customerRecordId)
					);
			...
			if (cancellation_reason == 28 && !isNullorEmpty(new_customer_id)) {
				//Change of Entity
				form.addButton(
					"custpage_movedigitalisation",
					"Move Digitalisation",
					getButtonScript("move_digitalisation", null, customerRecordId)
				);
			}
			//if not SignStatus
		} else {
			...
			if (
				(isInArray(userRole, salesRoles) ||
					isInArray(userRole, salesAdmin) ||
					isInArray(userRole, systemAdmin) ||
					isInArray(userRole, leadGenTeam)) &&
				status != 59 &&
				status != 22 &&
				status != 64
			) {
				form.addButton(
					"custpage_mark_duplicate_lost",
					"Mark Lost - Duplicate Lead",
					getButtonScript(
						"customer_mark_duplicate_lost",
						"null",
						customerRecordId
					)
				);
			...
			form.addButton(
				"custpage_lead_cancellation",
				"Cancel Lead",
				getButtonScript("lead_cancellation", null, customerRecordId)
			);
			...
			if (isInArray(userRole, salesRoles) || isInArray(userRole, salesAdmin)) {
				...
				filters[1] = new nlobjSearchFilter(
					...
					"is",
					[1, 2]
				); //campaignrecordtype is 'Prospects'
				...
				if (results == null) {
					form.addButton(
						"custpage_createsalesrecord",
						"Create Sales Record",
						getButtonScript("cr8salesrec", null, customerRecordId)
					);
					// if (userRole == 3) {
					...
					for (var i = 0; results != null && i < results.length; i++) {
						...
						var campaignrecordtype = res.getText(
							"custrecord_salescampaign_recordtype",
							"custrecord_sales_campaign"
						);
						var callbackdate = res.getValue("custrecord_sales_callbackdate");
					...
					if (isInArray(userRole, systemAdmin)) {
						form.addButton(
							"custpage_callcentre",
							"Call Centre",
							getButtonScript("xcallcentre", salesrecordid, customerRecordId)
						);
					}
					// }
					...
					form.addButton(
						"custpage_customer_reassign_sales_record",
						"Reassign Sales Record",
						getButtonScript(
							"reassignsalesrecord",
							salesrecordid,
							customerRecordId
					...
					if (
						...
						if (campaign == 56) {
							form.addButton(
								"custpage_unity4",
								"Start Unity4 Trial",
								getButtonScript("unity4", salesrecordid, customerRecordId)
							);
						}
						...
						if (
							today -
								nlapiAddDays(
									nlapiStringToDate(lastmodified),
									daysToNonExclusive
								) >=
							0
						) {
...
function getButtonScript(type, salesrecordid, customerrecordid) {
	...
	if (type == "callcentre") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_salescampaign",
			"customdeploy_sl_salescampaign"
		);
	...
	if (type == "cr8salesrec") {
		var url = nlapiResolveURL(
			...
			"customdeploy_sl_sales_campaign_popup"
		);
	...
	if (type == "reassignsalesrecord") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_create_sales_record",
			"customdeploy1"
		);
		url +=
			"&callCenter=T&customerId=" +
	...
	if (type == "customer_shipmate_access") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_shipmate_access",
			"customdeploy1"
		);
	...
	if (type == "create_sales_record_new") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_qualification_page_tn_v2",
			"customdeploy_sl_qualification_page_tn_d1"
		);
		url += "&recid=" + customerrecordid + "&button=T&cust=T";
		rtnScript = "window.location='" + url + "'";
	}
	...
	if (type == "maapgen") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_salesbtns_maapgen",
			"customdeploy_sl_salesbtns_maapgen"
		);
	...
	if (type == "commencement") {
		var url = nlapiResolveURL(
			...
			"customdeploy_sl_salesbtns_upload_file"
		);
		url +=
			"&recid=" +
			customerrecordid +
	...
	if (type == "finalisexsale") {
		...
		url +=
			"&customerId=" + customerrecordid + "&salesRecordId=" + salesrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	...
	if (type == "product") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_salesbtns_product",
			"customdeploy_sl_salesbtns_product"
		);
	...
	if (type == "unity4") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_unity4_comm_page",
			"customdeploy_sl_unity4_comm_page"
		);
		url += "&recid=" + customerrecordid + "&sales_record_id=" + salesrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	...
	if (type == "customer_on_boarding") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_customer_on_boarding",
			"customdeploy_sl_customer_on_boarding"
		);
	...
	if (type == "customer_cancellation") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_customer_cancellation",
			"customdeploy1"
		);
	...
	if (type == "customer_send_email") {
		...
		var url = nlapiResolveURL(
			...
			"customdeploy_sl_send_email_module"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	if (type == "customer_mark_duplicate_lost") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_lost_duplicate_lead",
			"customdeploy1"
		);
	...
	if (type == "sync_from_prospectplus") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_cust_rec_sync_from_pp",
			"customdeploy1"
		);
		url +=
			"&customerInternalId=" +
	...
	if (type == "rta_sync_display_name") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_update_cust_name",
			"customdeploy1"
		);
		url += "&customerInternalId=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	...
	if (type == "resync_lpo") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_resync_lpo",
			"customdeploy1"
		);
		url += "&customerInternalId=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	if (type == "resync_lpo_sub_customer") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_resync_lpo_sub_customer",
			"customdeploy1"
		);
		url += "&customerInternalId=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	...
	if (type == "customer_edit") {
		...
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_lead_capture2",
			"customdeploy_sl_lead_capture2"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	...
	if (type == "activate_cc") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_cc_activate_cust_resync",
			"customdeploy1"
		);
		url += "&customerid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	...
	if (type == "customer_coe") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_copy_customer",
			"customdeploy_sl_copy_customer"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	if (type == "customer_change") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_customer_notification",
			"customdeploy1"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	...
	if (type == "customer_prod_pricing") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl2_prod_pricing_page",
			"customdeploy1"
		);
		url += "&customerid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	...
	if (type == "multisite_link") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_link_multisite",
			"customdeploy_sl_link_multisite"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	if (type == "retrieve_multisite") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_linked_multisite_info",
			"customdeploy_sl_linked_multisite_info"
		);
		url += "&custid=" + customerrecordid;
		rtnScript = "window.location='" + url + "'";
	}
	if (type == "order_product") {
		...
		var lpo_question = customer_record.getFieldValue(
			"custentity_ap_lpo_customer"
		);
		...
		if (lpo_question == 2) {
			var url = nlapiResolveURL(
				"SUITELET",
				"customscript_sl_auspost_create_order",
				"customdeploy_sl_auspost_create_order"
			);
			...
			var url = nlapiResolveURL(
				"SUITELET",
				"customscript_sl_lpo_verification",
				"customdeploy_sl_lpo_verification"
			);
			url += "&trial_page=F&customer_id=" + customerrecordid;
			rtnScript = "window.location='" + url + "'";
		}
	...
	if (type == "move_digitalisation") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_move_digitalisation",
			"customdeploy_sl_move_digitalisation"
		);
	...
	if (type == "changestatus") {
		var url = nlapiResolveURL(
			"SUITELET",
			"customscript_sl_sales_campaign_status",
			"customdeploy_sl_sales_campaign_status"
		);
		url += "&recid=" + customerrecordid + "&button=T&cust=T";
		rtnScript = "window.location='" + url + "'";
	}
